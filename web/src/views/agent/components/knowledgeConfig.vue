    ...
    >
      ...
      <div v-if="fileResList.length" class="knowledge-file">
        ...
        >
          ...
          <i
            v-show="n.hover && n.status !== 'loading'"
            class="el-icon-error file-icon del"
            @click="preDelete(n, i)"
          ></i>
        </p>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      headers: {
        Authorization:
          'Bearer ' +
          JSON.parse(localStorage.getItem('access_cert')).user.token,
        'X-User-Id': JSON.parse(localStorage.getItem('access_cert')).user
          .userInfo.uid,
  ...
  methods: {
    ...
    fileChange(file, fileList) {
      ...
      this.$set(this.fileResList, index, {
        name: file.name,
        fileId: '',
        status: 'loading',
      });
    ...
    async preDelete(n, i) {
      switch (n.status) {
        case 'success':
          this.$confirm(
            this.$t('createApp.deleteFileTips'),
            ...
            let res = await deleteKnowledgeFile({
              fileId,
              assistantId: this.assistantId,
            });
            if (res.code === 0) {
